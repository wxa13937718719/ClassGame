(function () {
    ...
    async function loadRequiredChapters() {
        const manifest = await ChemData.loadManifest();
        let items = [];
        ...
        await Promise.all(items.map(item => ChemData.loadChapter(item.id)));

        items.forEach(item => {
            const chapter = window.CHEM_CHAPTERS[item.id];
            const validation = ChemValidator.reportChapter(chapter);
            if (!validation.valid) {
    ...
    function resolveRefs(refs) {
        const output = [];
        refs.forEach(ref => {
            const chapter = window.CHEM_CHAPTERS[ref.chapterId];
    ...
    function prepareQuestions() {
        if (state.mode === "chapter") {
            const chapter = window.CHEM_CHAPTERS[state.chapterId];
            state.questions = chapter.questions.map(question => questionWithChapter(chapter, question));
        } else if (state.mode === "wrong") {
            state.questions = resolveRefs(ChemStorage.getWrongBook());
        } else {
            state.questions = resolveRefs(ChemStorage.getFavorites());
        }
        state.questions = ChemEffects.shuffle(state.questions);
    }
    ...
    function setAutoRemoveWrong(enabled, notifyParent = false) {
        ...
        ChemStorage.setRemoveWrongOnCorrect(state.autoRemoveWrong);
    ...
    function updateFavoriteButton(question) {
        const favorite = ChemStorage.isFavorite(question.chapterId, question.id);
    ...
    function toggleFavorite() {
        ...
        if (!question) return;
        if (ChemStorage.isFavorite(question.chapterId, question.id)) {
            ChemStorage.removeFavorite(question.chapterId, question.id);
        } else {
            ChemStorage.addFavorite(question.chapterId, question.id);
        }
    ...
    function showQuestion() {
        ...
        state.displayedOptions = type === "fill" ? [] : ChemEffects.shuffle(question.options || []);
        updateProgress();
        updateFavoriteButton(question);
        ...
        ChemEffects.restartAnimation(el["game-card"], "slideIn 0.4s ease-out");
    }
    ...
    function recordResult(correct, question) {
        if (correct) {
            ...
            ChemEffects.animateScore(el.score, oldScore, state.score);
            ChemEffects.fireConfetti(70);
            ChemAudio.playCorrect();
            if (state.mode === "wrong" && state.autoRemoveWrong) {
                ChemStorage.removeWrong(question.chapterId, question.id);
            }
        } else {
            ChemEffects.shake(el["game-card"]);
            ChemAudio.playWrong();
            ChemStorage.addWrong(question.chapterId, question.id);
        }
    }
    ...
    function finishGame() {
        ...
        if (state.mode === "chapter") {
            ChemStorage.saveChapterResult(state.chapterId, {
                score: state.score,
    ...
    async function init() {
        cacheElements();
        getParams();
        state.autoRemoveWrong = ChemStorage.isRemoveWrongOnCorrect();
