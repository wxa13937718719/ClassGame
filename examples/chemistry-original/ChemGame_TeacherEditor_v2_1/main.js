...
app.setAppUserModelId(editorMode ? "OpenAI.ChemGame.Editor" : "OpenAI.ChemGame.Classroom");
...
function createEditorWindow(port) {
    ...
    const win = new BrowserWindow({
        ...
        backgroundColor: "#f4f6f8", title: "ChemGame 题库编辑器",
        webPreferences: {
...
if (!gotLock) {
    ...
    app.whenReady().then(async () => {
        try {
            ...
            dialog.showErrorBox(editorMode ? "ChemGame 编辑器启动失败" : "趣味化学闯关启动失败", `${error.message}\n\n日志文件：${logFile}`);
            app.quit();
        }
    });
}
