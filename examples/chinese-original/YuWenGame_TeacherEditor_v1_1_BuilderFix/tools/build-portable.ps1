...
$AppFolderName = "YuWenGame_Teacher_Windows_x64"
...
Set-Content -Path (Join-Path $AppDir "userdata\DO_NOT_DELETE.txt") -Encoding UTF8 -Value "YuWenGame learning records are stored in this folder."
Set-Content -Path (Join-Path $AppDir "backups\README.txt") -Encoding UTF8 -Value "YuWenGameEditor automatically keeps up to 20 question-bank backups here."

$ElectronExe = Join-Path $AppDir "electron.exe"
$GameExe = Join-Path $AppDir "YuWenGame.exe"
$EditorExe = Join-Path $AppDir "YuWenGameEditor.exe"
Rename-Item -Path $ElectronExe -NewName "YuWenGame.exe"

$LauncherCode = @"
using System;
using System.Diagnostics;
using System.IO;
public static class YuWenGameEditorLauncher {
    [STAThread]
    public static void Main() {
        string root = AppDomain.CurrentDomain.BaseDirectory;
        string exe = Path.Combine(root, "YuWenGame.exe");
...
$BuildInfo = @"
YuWenGame Teacher Portable Edition
Build time: $(Get-Date -Format "yyyy-MM-dd HH:mm:ss")
Electron: $Version
Platform: Windows x64

Game: YuWenGame.exe
Editor: YuWenGameEditor.exe
