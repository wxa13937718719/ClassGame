(function () {
    ...
    function setMainDirty(value) {
        if (window.yuwenEditor?.setDirty) window.yuwenEditor.setDirty(Boolean(value));
    }
    ...
    async function chooseQuestionImage() {
        const q = currentQuestion(); if (!q) return;
        const result = await window.yuwenEditor.chooseImage({ chapterId: state.selectedChapterId, questionId: q.id, kind: "question" });
    ...
    async function chooseOptionImage(question, option) {
        const result = await window.yuwenEditor.chooseImage({
    ...
    async function saveAll(options = {}) {
        ...
        try {
            ...
            const result = await window.yuwenEditor.saveData({ manifest: state.manifest, chapters: state.chapters });
            markSaved();
            if (!options.quiet) {
    ...
    async function loadData(askIfDirty = false) {
        ...
        try {
            const payload = await window.yuwenEditor.loadData();
    ...
    function bindActions() {
        ...
        el["open-game-btn"].addEventListener("click", async () => {
            ...
            await window.yuwenEditor.openGame();
        });
        ...
        if (window.yuwenEditor?.onSaveBeforeClose) {
            window.yuwenEditor.onSaveBeforeClose(async () => {
                const success = await saveAll({ quiet: true });
                await window.yuwenEditor.closeAfterSave(success);
            });
        }
    }
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        cacheElements();
        if (!window.yuwenEditor) {
            alert("编辑器需要从 YuWenGameEditor.exe 启动，不能直接双击 editor.html。");
            return;
        }
        bindActions();
