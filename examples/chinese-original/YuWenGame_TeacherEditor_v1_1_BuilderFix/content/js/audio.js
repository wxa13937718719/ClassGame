(function () {
    ...
    function applyEnabled(value, persist = true) {
        ...
        if (persist && window.YuWenStorage) {
            YuWenStorage.setAudioEnabled(enabled);
        }
        ...
        updateButton();
        document.dispatchEvent(new CustomEvent("yuwen-audio-change", { detail: { enabled } }));
    }

    function setEnabled(value) {
        if (isEmbeddedGame()) {
            window.parent.postMessage({ type: "yuwengame:set-audio", enabled: Boolean(value) }, "*");
            return;
        }
    ...
    function toggle() {
        if (isEmbeddedGame()) {
            window.parent.postMessage({ type: "yuwengame:toggle-audio" }, "*");
            return;
        }
        setEnabled(!enabled);
    }
    ...
    function bindParentAudioBridge() {
        ...
        window.addEventListener("message", event => {
            ...
            if (data.type !== "yuwengame:audio-state") return;
            enabled = data.enabled !== false;
            bgmStarted = enabled;
            updateButton();
        });

        window.parent.postMessage({ type: "yuwengame:request-audio-state" }, "*");
    }
    ...
    function init(options) {
        ...
        enabled = window.YuWenStorage ? YuWenStorage.isAudioEnabled() : true;
