(function () {
    ...
    async function loadRequiredChapters() {
        const manifest = await YuWenData.loadManifest();
        let items = [];
        ...
        await Promise.all(items.map(item => YuWenData.loadChapter(item.id)));

        items.forEach(item => {
            const chapter = window.YUWEN_CHAPTERS[item.id];
            const validation = YuWenValidator.reportChapter(chapter);
            if (!validation.valid) {
    ...
    function resolveRefs(refs) {
        const output = [];
        refs.forEach(ref => {
            const chapter = window.YUWEN_CHAPTERS[ref.chapterId];
    ...
    function prepareQuestions() {
        if (state.mode === "chapter") {
            const chapter = window.YUWEN_CHAPTERS[state.chapterId];
            state.questions = chapter.questions.map(question => questionWithChapter(chapter, question));
        } else if (state.mode === "wrong") {
            state.questions = resolveRefs(YuWenStorage.getWrongBook());
        } else {
            state.questions = resolveRefs(YuWenStorage.getFavorites());
        }
        state.questions = YuWenEffects.shuffle(state.questions);
    }
    ...
    function setAutoRemoveWrong(enabled, notifyParent = false) {
        ...
        YuWenStorage.setRemoveWrongOnCorrect(state.autoRemoveWrong);
        updateAutoRemoveControl();
        if (notifyParent && isEmbedded()) {
            window.parent.postMessage({ type: "yuwengame:set-auto-remove", enabled: state.autoRemoveWrong }, "*");
        }
    }
    ...
    function setPageTitle() {
        if (state.mode === "chapter") {
            ...
            el["mode-label"].textContent = "篇章闯关";
            document.title = `${chapter.title} - 语文素养闯关`;
        } else if (state.mode === "wrong") {
            el["chapter-title"].textContent = "错题笺";
            el["mode-label"].textContent = "错题订正";
            document.title = "错题笺 - 语文素养闯关";
        } else {
            el["chapter-title"].textContent = "佳题收藏";
            el["mode-label"].textContent = "佳题温习";
            document.title = "佳题收藏 - 语文素养闯关";
        }
        updateAutoRemoveControl();
    }
    ...
    function updateFavoriteButton(question) {
        const favorite = YuWenStorage.isFavorite(question.chapterId, question.id);
        el["favorite-btn"].classList.toggle("is-favorite", favorite);
        el["favorite-btn"].textContent = favorite ? "★ 已收藏" : "☆ 收藏本题";
        el["favorite-btn"].title = favorite ? "点击取消收藏" : "点击加入佳题收藏";
    }

    function toggleFavorite() {
        const question = state.questions[state.currentIndex];
        if (!question) return;
        if (YuWenStorage.isFavorite(question.chapterId, question.id)) {
            YuWenStorage.removeFavorite(question.chapterId, question.id);
        } else {
            YuWenStorage.addFavorite(question.chapterId, question.id);
        }
    ...
    function showQuestion() {
        ...
        state.displayedOptions = type === "fill" ? [] : YuWenEffects.shuffle(question.options || []);
        updateProgress();
        updateFavoriteButton(question);
        ...
        el["next-btn"].textContent = state.currentIndex === state.questions.length - 1 ? "查看结果 ➜" : "下一题 ➜";
        ...
        YuWenEffects.restartAnimation(el["game-card"], "slideIn 0.4s ease-out");
    }
    ...
    function getDisplayedAnswerText(question) {
        ...
        if (questionType(question) === "multiple") {
            return `本题正确选项：${items.map(item => `${item.letter}. ${item.option.content || ""}`).join("；")}`;
        }
        const item = items[0];
        return `本题正确选项：${item?.letter || "?"}. ${item?.option?.content || ""}`;
    }
    ...
    function recordResult(correct, question) {
        if (correct) {
            ...
            YuWenEffects.animateScore(el.score, oldScore, state.score);
            YuWenEffects.fireConfetti(70);
            YuWenAudio.playCorrect();
            if (state.mode === "wrong" && state.autoRemoveWrong) {
                YuWenStorage.removeWrong(question.chapterId, question.id);
            }
        } else {
            YuWenEffects.shake(el["game-card"]);
            YuWenAudio.playWrong();
            YuWenStorage.addWrong(question.chapterId, question.id);
        }
    }
    ...
    function finishGame() {
        ...
        if (state.mode === "chapter") {
            YuWenStorage.saveChapterResult(state.chapterId, {
                score: state.score,
        ...
        el["result-title"].textContent = state.mode === "chapter" ? "🎉 本卷完成！" : "🎉 温习完成！";
    ...
    function retryGame() {
        ...
        if (!state.questions.length) {
            if (state.mode === "wrong") showEmpty("错题笺已清空", "当前没有待订正的题目。 ");
            else if (state.mode === "favorite") showEmpty("佳题收藏为空", "当前没有已收藏的佳题。 ");
            return;
        }
    ...
    function backToDirectory() {
        if (isEmbedded()) window.parent.postMessage({ type: "yuwengame:back-menu" }, "*");
    ...
    function bindParentSettingBridge() {
        ...
        window.addEventListener("message", event => {
            ...
            if (data.type === "yuwengame:auto-remove-state") setAutoRemoveWrong(data.enabled !== false, false);
        });
        window.parent.postMessage({ type: "yuwengame:request-auto-remove-state" }, "*");
    }

    async function init() {
        cacheElements();
        getParams();
        state.autoRemoveWrong = YuWenStorage.isRemoveWrongOnCorrect();
        ...
        try {
            ...
            if (!state.questions.length) {
                if (state.mode === "wrong") showEmpty("错题笺是空的", "篇章练习中答错的题会自动出现在这里。 ");
                else if (state.mode === "favorite") showEmpty("佳题收藏为空", "答题时点击“收藏本题”，即可加入佳题收藏。 ");
                else showEmpty("本篇暂无题目", "请在教师编辑器中添加题目并保存发布。 ");
                return;
            }
            showQuestion();
        } catch (error) {
