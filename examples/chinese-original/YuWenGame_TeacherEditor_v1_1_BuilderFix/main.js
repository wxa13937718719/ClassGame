...
app.setAppUserModelId(editorMode ? "OpenAI.YuWenGame.Editor" : "OpenAI.YuWenGame.Classroom");
...
function createGameWindow(port, preview = false) {
    ...
    const options = {
        ...
        backgroundColor: "#3a2823", title: preview ? "语文素养闯关 - 编辑器预览" : "语文素养闯关",
...
function createEditorWindow(port) {
    ...
    const options = {
        ...
        backgroundColor: "#f6f0ea", title: "YuWenGame 语文题库编辑器",
        webPreferences: {
...
if (!gotLock) {
    ...
    app.whenReady().then(async () => {
        try {
            ...
            dialog.showErrorBox(editorMode ? "YuWenGame 编辑器启动失败" : "语文素养闯关启动失败", `${error.message}\n\n日志文件：${logFile}`);
            app.quit();
        }
    });
}
