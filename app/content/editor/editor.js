(function () {
    ...
    function setMainDirty(value) {
        if (window.classGameEditor?.setDirty) window.classGameEditor.setDirty(Boolean(value));
    }
    ...
    async function chooseQuestionImage() {
        const q = currentQuestion(); if (!q) return;
        const result = await window.classGameEditor.chooseImage({ chapterId: state.selectedChapterId, questionId: q.id, kind: "question" });
    ...
    async function chooseOptionImage(question, option) {
        const result = await window.classGameEditor.chooseImage({
    ...
    async function saveAll(options = {}) {
        ...
        try {
            ...
            const result = await window.classGameEditor.saveData({ manifest: state.manifest, chapters: state.chapters });
            markSaved();
            if (!options.quiet) {
    ...
    async function loadData(askIfDirty = false) {
        ...
        try {
            const payload = await window.classGameEditor.loadData();
    ...
    function bindActions() {
        ...
        el["open-game-btn"].addEventListener("click", async () => {
            ...
            await window.classGameEditor.openGame();
        });
        ...
        if (window.classGameEditor?.onSaveBeforeClose) {
            window.classGameEditor.onSaveBeforeClose(async () => {
                const success = await saveAll({ quiet: true });
                await window.classGameEditor.closeAfterSave(success);
            });
        }
    }
