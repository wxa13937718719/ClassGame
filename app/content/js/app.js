(function () {
    ...
    function createChapterCard(item) {
        const record = ClassGameStorage.getChapterRecord(item.id);
    ...
    function renderMenu() {
        ...
        const manifest = (window.CLASSGAME_CHAPTER_MANIFEST || []).slice().sort((a, b) => a.order - b.order);
        manifest.forEach(item => elements.chapterList.appendChild(createChapterCard(item)));

        const autoRemoveEnabled = ClassGameStorage.isRemoveWrongOnCorrect();
        elements.utilityList.appendChild(createUtilityCard({
            ...
            description: autoRemoveEnabled
                ? "答对后自动移除（可在上方关闭）"
                : "答对后继续保留（可在上方开启自动移除）",
            count: ClassGameStorage.getWrongBook().length,
        ...
        bindGameLaunches();
        ClassGameAudio.attachHover(".hover-target");
    }
    ...
    function bindToolbar() {
        ...
        elements.importInput.addEventListener("change", async event => {
            try {
                await ClassGameStorage.importState(event.target.files[0]);
                // 导入后同步本次运行中的声音开关状态。
                ClassGameAudio.applyExternalState(ClassGameStorage.isAudioEnabled());
                renderMenu();
        ...
        elements.autoRemoveButton.addEventListener("click", () => {
            const enabled = ClassGameStorage.toggleRemoveWrongOnCorrect();
            renderMenu();
            showMessage(
        ...
        document.getElementById("reset-btn").addEventListener("click", () => {
            ...
            if (!confirmed) return;
            ClassGameStorage.reset();
            ClassGameAudio.applyExternalState(true);
            renderMenu();
    ...
    async function loadCatalog() {
        await ClassGameData.loadEnabledChapters();
    }
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        // 每次重新打开程序时，默认将声音设为开启
        ClassGameStorage.setAudioEnabled(true);
